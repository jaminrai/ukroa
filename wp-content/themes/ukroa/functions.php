<?php
/**
 * UKROA functions and definitions
 *
 * @package UKROA
 */
	        	...
	        	<div class="dashboard-hero-panel">
		            <div class="hero-panel-content">
		                ...
		                <div class="hero-panel-title">
		                    ...
		                    printf(esc_html__('Welcome to', 'UKROA') . ' %s', $theme_name ); ?>
		                </div>
...
function ukroa_register_slide_cpt() {
    $labels = array(
        ...
        'set_featured_image'    => __( 'Set slide image', 'text_domain' ),
        'remove_featured_image' => __( 'Remove slide image', 'text_domain' ),
    ...
    $args = array(
        ...
        'description'           => __( 'Slides for UKROA homepage slider', 'text_domain' ),
        'labels'                => $labels,
        'supports'              => array( 'title', 'excerpt', 'thumbnail', 'page-attributes' ), // page-attributes for menu_order sorting
        'hierarchical'          => false,
        'public'                => false, // Not public-facing, only for admin
        'show_ui'               => true,
        'show_in_menu'          => true,
        'menu_position'         => 20,
    ...
    register_post_type( 'ukroa_slide', $args );
}
add_action( 'init', 'ukroa_register_slide_cpt', 0 );

// Enable theme support for post thumbnails if not already
if ( ! current_theme_supports( 'post-thumbnails' ) ) {
    add_theme_support( 'post-thumbnails' );
}
// Add Metabox for Slide Link and Link Text
function ukroa_slide_metabox() {
    add_meta_box(
        'ukroa_slide_details',
        __( 'Slide Link Details', 'text_domain' ),
        'ukroa_slide_metabox_callback',
        'ukroa_slide' // Attach to our CPT
    );
}
add_action( 'add_meta_boxes', 'ukroa_slide_metabox' );

// Callback to display the metabox fields
function ukroa_slide_metabox_callback( $post ) {
    wp_nonce_field( 'ukroa_slide_nonce', 'ukroa_slide_nonce' );

    $link_url = get_post_meta( $post->ID, '_ukroa_slide_link_url', true );
    $link_text = get_post_meta( $post->ID, '_ukroa_slide_link_text', true );

    echo '<p><label for="ukroa_slide_link_url">' . __( 'Link URL (e.g., /about-us):', 'text_domain' ) . '</label><br>';
    echo '<input type="url" id="ukroa_slide_link_url" name="ukroa_slide_link_url" value="' . esc_attr( $link_url ) . '" size="50" /></p>';

    echo '<p><label for="ukroa_slide_link_text">' . __( 'Link Text (Button Text, e.g., Learn More):', 'text_domain' ) . '</label><br>';
    echo '<input type="text" id="ukroa_slide_link_text" name="ukroa_slide_link_text" value="' . esc_attr( $link_text ) . '" size="50" /></p>';
}

// Save the metabox data
function ukroa_save_slide_metabox( $post_id ) {
    if ( ! isset( $_POST['ukroa_slide_nonce'] ) ) {
        return;
    }
    if ( ! wp_verify_nonce( $_POST['ukroa_slide_nonce'], 'ukroa_slide_nonce' ) ) {
        return;
    }
    ...
    if ( isset( $_POST['ukroa_slide_link_url'] ) ) {
        update_post_meta( $post_id, '_ukroa_slide_link_url', esc_url_raw( $_POST['ukroa_slide_link_url'] ) );
    }
    if ( isset( $_POST['ukroa_slide_link_text'] ) ) {
        update_post_meta( $post_id, '_ukroa_slide_link_text', sanitize_text_field( $_POST['ukroa_slide_link_text'] ) );
    }
}
add_action( 'save_post', 'ukroa_save_slide_metabox' );
/*end of the slides in homepage*/
...
// ==== PERMANENTLY DISABLE UKROA THEME UPDATES ====
add_filter('site_transient_update_themes', 'ukroa_completely_disable_theme_updates');
function ukroa_completely_disable_theme_updates($updates) {
    if (isset($updates->response['ukroa'])) {
        unset($updates->response['ukroa']);           // Remove UKROA from update list
    }
    if (isset($updates->response['newscrunch'])) {
        unset($updates->response['newscrunch']);      // Also block parent theme
    }
    return $updates;
}

// Optional: Hide the update nags in dashboard too
